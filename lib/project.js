(() => {
  ...
  function walkFiberForUrl(startEl) {
    ...
    const rootFiber = startEl[fiberKey];
    ...
    while (queue.length && steps < 120) {
      ...
      // Walk up a little for row wrappers that hold the session object
      if (steps < 30 && fiber.return) queue.push(fiber.return);
  ...
  async function loadAllSessionRows(doc = document, onProgress = () => {}) {
    ...
    const maxSteps = isLibraryPage(doc) ? 250 : 100;
    const settleAtBottom = isLibraryPage(doc) ? 8 : 5;
    const settleStuck = isLibraryPage(doc) ? 12 : 8;
    const stepDelay = isLibraryPage(doc) ? 450 : 350;
    ...
    try {
      ...
      let stable = 0;
      let lastCount = store.values().length;
      for (let i = 0; i < maxSteps; i += 1) {
        ...
        const nextCount = store.values().length;
        if (nextCount === lastCount) stable += 1;
        else {
          stable = 0;
          lastCount = nextCount;
        }
        ...
        if (atBottom && stable >= settleAtBottom) break;
        if (!atBottom && stable >= settleStuck) break;
      }
      ...
      // API harvest often yields UUIDs even when the table row has no href
