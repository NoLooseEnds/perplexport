(() => {
  ...
  function getProjectInfo(doc = document) {
    ...
    return { slug, name, path, url, kind: "project" };
  }
  ...
  /** Exact then unique long-prefix match — avoid binding the wrong similar title. */
  function lookupHarvestUrl(title, harvestByTitle) {
    if (!title || !harvestByTitle || !harvestByTitle.size) return null;
    const key = normalizeTitleKey(title);
    if (!key) return null;
    if (harvestByTitle.has(key)) return harvestByTitle.get(key);

    for (const n of [60, 48, 40]) {
      const prefix = key.slice(0, n);
      if (prefix.length < 32) break;
      const hits = [];
      for (const [k, url] of harvestByTitle) {
        if (!k || k.length < 32) continue;
        if (k.startsWith(prefix) || prefix.startsWith(k.slice(0, prefix.length))) {
          hits.push(url);
        }
      }
    ...
    const upsert = (item) => {
      ...
      if (item.url) {
        ...
        const merged = {
          ...
          projectTag: item.projectTag || existingByUrl?.projectTag || null,
          files: mergeFileLists(existingByUrl?.files, item.files),
        ...
        if (titleKey) {
          ...
          if (titled && !titled.url) {
            ...
            merged.projectTag = merged.projectTag || titled.projectTag;
            merged.files = mergeFileLists(merged.files, titled.files);
      ...
      if (existingByTitle?.url) {
        const merged = {
          ...
          projectTag: item.projectTag || existingByTitle.projectTag || null,
          files: mergeFileLists(existingByTitle.files, item.files),
  ...
  function installNetworkHarvester() {
    ...
    function walkPairs(data, depth) {
      ...
      if (Array.isArray(data)) {
        data.slice(0, 400).forEach((item) => walkPairs(item, depth + 1));
        return;
      }
      const title =
        data.title ||
  ...
  function rowTitle(row) {
    ...
    const titleCell = cells.length >= 2 ? cells[1] : null;
    if (titleCell) {
      // Prefer the dedicated title node — ignore project pills / file chips in the same cell
      const primary =
        titleCell.querySelector(".min-w-0.flex-1 .truncate") ||
        titleCell.querySelector(".min-w-0.flex-1 .text-sm") ||
        titleCell.querySelector(".min-w-0.flex-1") ||
        null;
      if (primary) {
        const clone = primary.cloneNode(true);
        clone
          .querySelectorAll(
            '[data-testid="task-collection-pill"], [data-files-cell-measure-item], [data-files-cell-measure-overflow], .group\\/pill, .group\\/asset, button'
          )
      ...
      // Fallback: first text line of the cell after stripping pills/files
      const cellClone = titleCell.cloneNode(true);
      cellClone
        .querySelectorAll(
          '[data-testid="task-collection-pill"], [data-files-cell-measure-item], [data-files-cell-measure-overflow], .group\\/pill, .group\\/asset, .ml-auto'
        )
        .forEach((el) => el.remove());
      const fromCell = cleanText(cellClone.innerText || cellClone.textContent)
        .split("\n")
        .map((l) => l.trim())
        .filter((l) => l.length >= 2)
        .filter((l) => !/^\+\d+$/.test(l))
        .filter((l) => !/\.(jpg|jpeg|png|gif|webp|pdf|docx?|xlsx?|csv|txt)$/i.test(l))
        .find((l) => l.length >= 8);
      if (fromCell) return fromCell;
    }

    const candidates = Array.from(row.querySelectorAll('[role="cell"] .text-sm.truncate, [role="cell"] .text-primary.truncate'))
  ...
  /** Project / collection pill on a Library Sessions row, if present. */
  function rowProjectTag(row) {
    const pill =
      row.querySelector('[data-testid="task-collection-pill"]') ||
      row.querySelector(".group\\/pill");
    if (!pill) return null;
    const clone = pill.cloneNode(true);
    clone.querySelectorAll("svg, .font-emoji, img").forEach((el) => el.remove());
    const text = cleanText(clone.innerText || clone.textContent);
    return text || null;
  }
  ...
  function mergeFileLists(...lists) {
    ...
    for (const list of lists) {
      for (const raw of list || []) {
        ...
        if (!isAttachmentName(name)) continue;
  ...
  /** File chips on a Library Sessions row (prefer measurement nodes — they list all files). */
  function rowFiles(row) {
    if (!row) return [];
    const names = [];
    const roots = row.querySelectorAll(
      "[data-files-cell-measure-item] .group\\/asset, .group\\/asset"
    );
    roots.forEach((asset) => {
      if (asset.closest('[data-testid="task-collection-pill"]')) return;
      const label =
        asset.querySelector(".truncate") ||
        asset.querySelector(".text-xs") ||
        asset;
      const clone = label.cloneNode(true);
      clone.querySelectorAll("svg, img").forEach((el) => el.remove());
      const text = cleanText(clone.innerText || clone.textContent);
      if (isAttachmentName(text)) names.push(text);
    });
    return mergeFileLists(names);
  }
  ...
  function collectFromSessionsTable(doc, extras = {}) {
    ...
    findSessionRows(scope).forEach((row) => {
      ...
      const projectTag = rowProjectTag(row);
      ...
      if (!url && title && harvestByTitle) {
        url = lookupHarvestUrl(title, harvestByTitle);
      }
      store.upsert({ url, title, date, projectTag, files, row });
    });
  ...
  async function ensureBackOnListPage(listHref) {
    ...
    // Prefer soft history — location.assign() full-reloads and aborts Find missing
    try {
      history.back();
    } catch {
      // ignore
    }
    const backDeadline = Date.now() + 6000;
    while (Date.now() < backDeadline) {
      if (isBulkListPage()) return true;
      if (listHref && location.href === listHref) return true;
      await sleep(120);
    }

    try {
      const target = listHref || "/library";
  ...
  async function resolveMissingThreadUrls(threads, onProgress = () => {}) {
    ...
    const missingTotal = threads.filter((t) => !t.url).length;
    let missingDone = 0;
    ...
    for (let i = 0; i < threads.length; i += 1) {
      ...
      onProgress({
        ...
        total: missingTotal,
        title: thread.title,
        done: resolved.filter((t) => t.url).length,
      });

      try {
        if (!isBulkListPage()) {
          const ok = await ensureBackOnListPage(listHref);
          if (!ok) {
            // Stay put — do not full-reload. Skip this row and keep going later.
            resolved.push({ ...thread, url: null, row: undefined });
        ...
        if (!url && liveRow) {
          url = await discoverUrlByOpeningRow(liveRow);
        }

        if (url) consecutiveFailures = 0;
        else consecutiveFailures += 1;

        resolved.push({ ...thread, url: url || null, row: undefined });
        missingDone += 1;

        // Persist partial progress so a later crash can still show links found
        if (missingDone % 5 === 0 || url) {
          onProgress({
            index: missingDone - 1,
            total: missingTotal,
            title: thread.title,
            partial: resolved.slice(),
          });
        }
  ...
  async function loadAllSessionRows(doc = document, onProgress = () => {}) {
    ...
    const bumpScroll = (scrollEl) => {
      if (!scrollEl) return { moved: false, before: 0, after: 0 };
      const before = scrollEl.scrollTop;
      const delta = Math.max(Math.floor(scrollEl.clientHeight * 0.65), 360);
      ...
      if (scrollEl.scrollTop === before) {
        ...
        fireScrollSignals(scrollEl, delta + 200);
      }
      for (const el of candidates) {
    ...
    const unstickScroll = async (scrollEl) => {
      if (!scrollEl) return;
      const top = scrollEl.scrollTop;
      // Tiny reverse bump often re-arms infinite scroll observers
      scrollEl.scrollTop = Math.max(0, top - 80);
      fireScrollSignals(scrollEl, -80);
      await sleep(120);
      scrollEl.scrollTop = top + Math.max(scrollEl.clientHeight, 500);
      fireScrollSignals(scrollEl, 900);
      await sleep(200);
      // Try every candidate briefly
      for (const el of candidates) {
        fireScrollSignals(el, 600);
    ...
    try {
      ...
      for (let i = 0; i < maxSteps; i += 1) {
        ...
        onProgress({
          rows: values.length,
          withUrl,
          harvested: harvester.byTitle.size,
          step: i,
          peak: peakCount,
          phase: atBottom ? "waiting" : stuckSteps > 2 ? "unsticking" : "scrolling",
          idle: bottomIdle,
          idleNeeded: bottomIdleNeeded,
        });

        const bump = bumpScroll(scrollEl);
      ...
      // Fuzzy-attach harvested URLs onto existing title-only rows
      for (const thread of store.values()) {
        if (thread.url || !thread.title) continue;
        const url = lookupHarvestUrl(thread.title, harvester.byTitle);
        if (url) store.upsert({ ...thread, url });
      }
  ...
  async function listProjectThreads(options = {}) {
    ...
    let threads;
    if (options.skipLoad && options.existingThreads?.length) {
      // Reuse the list from the prior scroll — Find missing should not
      // re-run the multi-minute Library crawl.
      const store = createThreadStore();
      for (const t of options.existingThreads) {
        store.upsert({
          title: t.title || "",
          url: t.url || null,
          date: t.date || null,
          projectTag: t.projectTag || t.project || null,
          files: Array.isArray(t.files) ? t.files.slice() : [],
        });
      }
      // Merge whatever is currently mounted (no scrolling)
      collectFromSessionsTable(doc).forEach((t) => store.upsert(t));
      threads = store.values();
      options.onProgress?.({
        rows: threads.length,
        withUrl: threads.filter((t) => t.url).length,
        phase: "cached",
      });
    } else {
      threads = await loadAllSessionRows(doc, options.onProgress);
    }

    if (options.resolveMissing) {
      const missingCount = threads.filter((t) => !t.url).length;
      if (missingCount) {
        threads = await resolveMissingThreadUrls(
          threads,
          options.onResolveProgress
        );
      }
    }
