(() => {
  ...
  function findListScrollers(table, doc = document) {
    const found = [];
    const add = (el) => {
      if (el && !found.includes(el) && isScrollable(el)) found.push(el);
    };

    const radix =
      table?.closest?.("[data-radix-scroll-area-viewport]") ||
      table?.querySelector?.("[data-radix-scroll-area-viewport]") ||
      doc.querySelector("main [data-radix-scroll-area-viewport]");
    add(radix);
  ...
  async function loadAllSessionRows(doc = document, onProgress = () => {}) {
    ...
    const maxSteps = isLibrary ? 800 : 150;
    const stepDelay = isLibrary ? 550 : 350;
    const bottomIdleNeeded = isLibrary ? 24 : 8;
    const midIdleNeeded = isLibrary ? 30 : 16;
    const bottomPause = isLibrary ? 900 : 400;

    const harvester = installNetworkHarvester();
    try {
      await harvester.ready;
    } catch {
      // harvest is optional — scrolling still works without it
    }
    ...
    const bumpScroll = () => {
      ...
      for (let i = 0; i < candidates.length; i += 1) {
        ...
        const delta =
          i === 0
            ? Math.max(Math.floor(el.clientHeight * 0.95), 600)
            : Math.max(Math.floor(el.clientHeight * 0.5), 300);
    ...
    try {
      ...
      for (let i = 0; i < maxSteps; i += 1) {
        ...
        const bottom = isAtBottom(scrollEl);
        ...
        if (bottom) {
          ...
          merge();
          const after = store.values().length;
          if (after > peakCount) {
            peakCount = after;
            idleAtPeak = 0;
            continue;
          }
          if (idleAtPeak >= bottomIdleNeeded) break;
        } else if (idleAtPeak >= midIdleNeeded) {
          if (scrollEl) scrollEl.scrollTop = scrollEl.scrollHeight;
          await sleep(bottomPause);
          merge();
          if (store.values().length <= peakCount) break;
          peakCount = store.values().length;
          idleAtPeak = 0;
        }
      }

      // Second pass — catch anything the first pass skipped
      if (scrollEl) {
        scrollEl.scrollTop = 0;
        await sleep(250);
        merge();
        idleAtPeak = 0;
        peakCount = Math.max(peakCount, store.values().length);
        for (let j = 0; j < (isLibrary ? 200 : 40); j += 1) {
          onProgress({
            rows: store.values().length,
            withUrl: store.values().filter((t) => t.url).length,
            step: `2.${j}`,
            peak: peakCount,
          });
          bumpScroll();
          await sleep(Math.min(stepDelay, 450));
          merge();
          const count = store.values().length;
          if (count > peakCount) {
            peakCount = count;
          ...
          if (isAtBottom(scrollEl)) {
            ...
            if (idleAtPeak >= Math.ceil(bottomIdleNeeded / 2)) break;
          }
        }
      }
