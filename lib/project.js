(() => {
  ...
  /** Exact then prefix match — DOM titles and API titles often differ slightly. */
  function lookupHarvestUrl(title, harvestByTitle) {
    ...
    for (const n of [60, 48, 40, 32, 24]) {
      ...
      for (const [k, url] of harvestByTitle) {
        ...
        if (
          k.startsWith(prefix) ||
          prefix.startsWith(k.slice(0, Math.min(prefix.length, k.length)))
        ) {
          return url;
        }
      }
