(() => {
  ...
  function digModelFields(value, depth = 0, seen) {
    ...
    if (Array.isArray(value)) {
      for (const item of value.slice(0, 30)) {
        const found = digModelFields(item, depth + 1, bag);
    ...
    for (const key of preferred) {
      if (value[key] && typeof value[key] === "object") {
        const found = digModelFields(value[key], depth + 1, bag);
        if (found) return found;
      }
    }
    return null;
  ...
  function getComposerModel(doc) {
    // Only look near the ask/composer control — never scan the whole page
    // (nav/sidebar text like "Perplexity" would otherwise pollute every turn).
    const textarea =
      qs(doc, "textarea") ||
      qs(doc, '[contenteditable="true"][role="textbox"]');
    const scope =
    ...
    const buttons = qsa(scope, "button, [role='button']").slice(0, 40);
    for (const btn of buttons) {
      const text = cleanText(btn.innerText || btn.textContent);
      if (!text || text.length > 40) continue;
      if (!MODEL_HINT_RE.test(text)) continue;
      if (/^(Ask|Search|Submit|Send|Model)$/i.test(text)) continue;
      return packModelInfo({ label: text });
    }
    return null;
  }
  ...
  function collectEntryModels(data) {
    ...
    function walk(node, depth) {
      if (depth > 14 || node == null || out.length >= 120) return;
  ...
  async function fetchThreadEntryModels() {
    try {
      ...
      if (!m) return [];
      // pathname capture is already decoded; avoid double-encoding the slug
      const slug = m[1];
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 8000);
      let res;
      try {
        res = await fetch(`${location.origin}/rest/thread/${slug}`, {
          credentials: "include",
  ...
  function resolveTurnModel({
    ...
    // Prefer API — skip expensive fiber/DOM when we already have a match
  ...
  function applyApiModelsToTurns(turns, entryModels) {
    if (!entryModels?.length || !turns?.length) return turns;
    return turns.map((turn, i) => {
      const fromApi = matchEntryModel(turn.query, i, entryModels);
      if (!fromApi) return turn;
      if (modelScore(fromApi) >= modelScore(turn.model)) {
        return { ...turn, model: fromApi };
      }
      return turn;
    });
  }
  ...
  function summarizeConversationModel(turns) {
    ...
    for (const entry of counts.values()) {
      if (
        !best ||
        entry.n > best.n ||
        modelScore(entry.model) > modelScore(best.model)
      ) {
        best = entry;
      }
    }
  ...
  function extractTurns(doc, entryModels = [], options = {}) {
    const resolveModels = options.resolveModels !== false;
    ...
    // Lazy: only scan composer UI if a turn actually needs a fallback
    let composerResolved = false;
    let composerModel = null;
    const getFallback = () => {
      if (!resolveModels) return null;
      if (!composerResolved) {
        composerResolved = true;
        composerModel = getComposerModel(doc);
      }
  ...
  async function extractConversation(doc = document, extras = {}) {
    ...
    const turns = applyApiModelsToTurns(
      extractTurns(doc, entryModels, { resolveModels: true }),
      entryModels
    );
    return buildConversation(doc, turns, extras);
  }
  ...
  async function loadFullConversation(options = {}) {
    ...
    // Defer model resolution until after scrolling — fiber/DOM scans on every
    // snapshot were freezing long threads during bulk export.
    const snapshot = () => {
      mergeTurns(store, extractTurns(doc, [], { resolveModels: false }));
      onProgress({
        phase: "loading",
        turns: store.size,
    ...
    // One model pass over currently mounted DOM + API enrichment for all turns
    mergeTurns(store, extractTurns(doc, entryModels, { resolveModels: true }));
    const turns = applyApiModelsToTurns(
