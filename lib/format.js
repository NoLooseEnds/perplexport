(() => {
  ...
  function modelLine(model) {
    ...
    if (
      model.selected &&
      model.display &&
      model.selected !== model.display
    ) {
      return `${model.label} (display: ${model.display})`;
    }
    return model.label;
  }
  ...
  function toMarkdown(conversation) {
    ...
    if (conversation.attachments?.length) {
      ...
      conversation.attachments.forEach((name) => {
        lines.push(`  - ${yamlEscape(name)}`);
      });
    }
    if (conversation.model?.label) {
      lines.push(`model: ${yamlEscape(conversation.model.label)}`);
      if (conversation.model.selected) {
        lines.push(
          `model_selected: ${yamlEscape(conversation.model.selected)}`
        );
      }
      if (
        conversation.model.display &&
        conversation.model.display !== conversation.model.selected
      ) {
        lines.push(
          `model_display: ${yamlEscape(conversation.model.display)}`
        );
      }
    }
    ...
    const convoModel = modelLine(conversation.model);
    if (convoModel) meta.push(`- **Model:** ${convoModel}`);
    ...
    conversation.turns.forEach((turn, idx) => {
      ...
      if (turn.answerHtml || turn.answerText) {
        lines.push(`## Answer ${n}`);
        const turnModel = modelLine(turn.model);
        if (turnModel) lines.push(`*Model: ${turnModel}*`);
        lines.push("");
  ...
  function serializeModel(model) {
    if (!model?.label && !model?.selected && !model?.display) return null;
    const out = {};
    if (model.label) out.label = model.label;
    if (model.selected) out.selected = model.selected;
    if (model.display) out.display = model.display;
    if (model.variants?.length) out.variants = model.variants.slice();
    return out;
  }
  ...
  function toJson(conversation) {
    const payload = {
      ...
      attachments: conversation.attachments?.length
        ? conversation.attachments.slice()
        : [],
      ...
      turns: (conversation.turns || []).map((turn, idx) => ({
        ...
        answer: turn.answerHtml
          ? htmlToMarkdown(turn.answerHtml)
          : turn.answerText || "",
        model: serializeModel(turn.model),
